#setwd("C:/Users/michael/Desktop/Scotus_Network_2/scotus/with_node_attributes/shiny_michael7_cum_log_total_deg")
...
shinyServer(function(input, output) {
  ...
  output$distPlot <- renderPlot({
      ...
      plot(log(as.integer(names(table(datasetInput())))), log(table(datasetInput())),
           main = paste('log-log plot of cumulative total-degree distribution up to ', input$yearA),
           xlim = c(0, 7.2), #from log log plot of degree distribution of entire network and log(1317) == 7.183112
           ylim = c(0, 8.5), #from log log plot of degree distribution of entire network
           xlab = paste('cumulative log(total-degree) up to ', input$yearA),
           ylab = 'log(counts)',
           col = "#ACA4E2"
      )
...
shinyUI(fluidPage(
  ...
  # Application title
  titlePanel("Log-Log Plot of Cumulative Total-Degree Distribution Up to Some Year of SCOTUS Network"),
